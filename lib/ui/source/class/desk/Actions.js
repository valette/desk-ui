	...
	*/
	construct : function() {
		...
		if (qx.bom.Cookie.get("homeURL")) {
			this.__socket = io({path : desk.FileSystem.getBaseURL() + 'socket/socket.io'});
			this.__rpcPresent = true;
		}

		try {
			this.__socket = require('electron').ipcRenderer;
			this.__emit = function (channel, message) {
				this.__socket.send(channel, message);
			};
			this.__on = function (event, callback) {
				return this.__socket.on(event, function (event, message) {
					callback(message);
				});
			};
			this.__rpcPresent = true;
		...
		this.__on("action finished", this.__onActionEnd.bind(this));
		this.__on("actions updated", this.__populateActionMenu.bind(this));
	...
	statics : {
		...
		*/
		execute : function (params, options, callback, context) {
			...
			if (options.listener) {
				params.stream = true;
				parameters.listener = desk.Actions.getInstance().__on("actionEvent",
					function (message) {
	...
	members : {
		...
		__emit : function (channel, message) {
			this.__socket.emit(channel, message);
		},

		__on : function (event, callback) {
			return this.__socket.on(event, callback);
		},
		...
		*/
		__createActionsMenu : function () {
			...
			this.__on("loadavg", function (loadavg) {
		...
		*/
		__getServerLogButton : function () {
			...
			button.addListener('execute', function () {
				...
				win.add(log, {flex : 1});
				this.__emit('setEmitLog', true);
				this.__on("log", displayLog);
				win.addListener('close', function () {
					this.__socket.removeListener('log', displayLog);
					this.__emit('setEmitLog', false);
				}, this);
				win.open();
				win.center();
			}, this);
