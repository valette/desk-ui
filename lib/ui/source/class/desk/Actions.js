	...
	*/
	construct : function() {
		if (qx.bom.Cookie.get("homeURL")) {
			this.__socket = io(baseURL + 'socket/socket.io');
		...
		try {
			...
			this.__socket.emit = this.__socket.send;
			console.log(this.__socket);
			this.__rpcPresent = true;
			this.__socket.on("action finished", function (event, message) {
				console.log(event);
				console.log(message);
			});
			this.__socket.on("test", function (event, message) {
				console.log(event);
				console.log(message);
			});
